...
async function hasShownNoticeBefore(anonymousId) {
  try {
    ...
    const baseDir = process.platform === "win32" ? process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming") : path.join(os.homedir(), ".config");
...
async function createTelemetry(options, context) {
  const debugEnabled = options.telemetry?.debug || env.getBooleanEnvVar("BETTER_AUTH_TELEMETRY_DEBUG");
  const disableNotice = options.telemetry?.disableNotice || env.getBooleanEnvVar("BETTER_AUTH_TELEMETRY_DISABLE_NOTICE");
